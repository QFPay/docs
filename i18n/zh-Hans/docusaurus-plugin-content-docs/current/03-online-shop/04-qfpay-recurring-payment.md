...
3. [对接我们的 element 服务](/docs/online-shop/paymentelement#引入方式) 并使用 elements.createEnhance() 和 payment.pay() 来构建 token 创建页面. 然后使用 `customer_id` 和卡信息来创建 token 并获得 `token_id`。
