...
:::tip
If `lang` is not passed, Element will use the browser’s default language.
:::

:::warning
The Element container must not be placed inside a `<form>` element or rendering will break.
:::

:::info
In production, the payment intent expires in 2 years. In sandbox: 7 days.
:::
